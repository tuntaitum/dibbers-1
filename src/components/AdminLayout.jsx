...
import { noiseOverlayDark, adminSidebarBg } from "@/lib/portalDesign";
...
export default function AdminLayout() {
  ...
  return (
    <div className="flex min-h-screen" style={{ background: "#F7F5F0" }}>
      {/* Desktop Sidebar — dark gradient color block with noise */}
      <aside className="hidden md:flex flex-col w-64 min-h-screen fixed left-0 top-0 z-40" style={adminSidebarBg}>
        ...
        <div className="relative z-10 p-6 border-b border-white/10">
          ...
          <p className="text-white/40 text-xs mt-0.5 font-body font-light tracking-[0.15em] uppercase">Admin Panel</p>
        </div>
      ...
      {/* Mobile Nav */}
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 px-4 py-3 flex items-center justify-between" style={adminSidebarBg}>
        <div className="absolute inset-0 pointer-events-none" style={noiseOverlayDark} />
        <span className="relative font-heading text-xl font-bold text-white tracking-[-0.02em]">Dibbers Admin</span>
        <button onClick={() => setMobileOpen(!mobileOpen)} className="relative text-white p-1">
          {mobileOpen ? <X size={22} /> : <Menu size={22} />}
        </button>
      </div>
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-40 pt-16 px-4" style={adminSidebarBg}>
          <div className="absolute inset-0 pointer-events-none" style={noiseOverlayDark} />
          {navItems.map(({ path, label }) => (
