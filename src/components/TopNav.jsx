...
export default function TopNav({ items = [], logoLink = "/explore", activeBg = "bg-brand-orange", showSignIn = false }) {
  ...
  return (
    <nav className="sticky top-4 z-50 mx-auto max-w-6xl px-3 md:px-4">
      <div className="relative rounded-2xl px-3 md:px-4 py-2.5 overflow-hidden" style={frostedCard}>
          ...
          {items.length > 0 && (
            <div className="flex items-center gap-1 overflow-x-auto no-scrollbar flex-1 min-w-0">
              {items.map(({ path, label, icon: Icon }) => {
                ...
                return (
                  <Link
                    key={path}
                    to={path}
                    className={`flex-shrink-0 flex items-center gap-1.5 px-3 md:px-4 py-2 rounded-xl text-xs md:text-sm font-semibold transition-all ${
                      active
                        ? `${activeBg} text-white`
              ...
              {showSignIn && !isAuthenticated && (
                <Link
                  to="/login"
                  className="flex-shrink-0 ml-1 flex items-center gap-1.5 px-3 md:px-4 py-2 rounded-xl text-xs md:text-sm font-semibold bg-[#1a1a1a] text-white hover:bg-brand-orange transition-all"
                >
                  <LogIn size={15} />
                  Sign in
                </Link>
