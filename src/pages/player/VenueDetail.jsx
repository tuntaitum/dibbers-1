...
export default function VenueDetail() {
  ...
  const loadingShell = isMobile ? (
    <div className="min-h-screen flex items-start justify-center overflow-x-hidden" style={{ background: "#FAFAFA" }}>
      <div className="w-full max-w-[430px] min-h-screen flex items-center justify-center shadow-2xl overflow-x-hidden" style={{ background: "#FAFAFA" }}>
  ...
  if (!venue) return isMobile ? (
    <div className="min-h-screen flex items-start justify-center overflow-x-hidden" style={{ background: "#FAFAFA" }}>
      <div className="w-full max-w-[430px] min-h-screen flex flex-col items-center justify-center gap-3 shadow-2xl overflow-x-hidden" style={{ background: "#FAFAFA" }}>
  ...
  return (
    <div className="min-h-screen flex items-start justify-center overflow-x-hidden" style={{ background: "#FAFAFA" }}>
      <div className="w-full max-w-[430px] min-h-screen pb-8 shadow-2xl overflow-x-hidden" style={{ background: "#FAFAFA" }}>
        ...
        <div className="relative mt-4">
          {venue.photos?.[0] ? (
            ...
            <div className="w-full h-56 bg-brand-brown flex items-center justify-center">
              <span className="text-6xl">🏟️</span>
            </div>
          )}
          <button onClick={() => navigate(-1)} className="absolute top-4 left-4 bg-white/90 p-2 rounded-full shadow">
            <ArrowLeft size={18} className="text-brand-brown" />
          </button>
        </div>
        ...
        <div className="px-4 py-5">
          ...
          <div className="flex items-start justify-between gap-2 mb-3">
            <div>
              <h1 className="font-heading text-3xl font-bold text-brand-brown">{venue.name}</h1>
              ...
              <div className="grid grid-cols-2 gap-2">
                {slots.map(slot => (
                  <button
                    ...
                    className="bg-white border border-border rounded-xl p-3 text-left hover:border-brand-orange hover:bg-brand-orange/5 transition-all active:scale-[0.98] group"
                  >
