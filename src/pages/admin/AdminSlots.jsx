...
export default function AdminSlots() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "#FAFAFA" }}>
      ...
      <div className="px-6 md:px-10 py-4 space-y-5">
        {/* Filters panel */}
        {loading ? (
          ...
          <div className="rounded-2xl p-5 space-y-4" style={frostedCard}>
            ...
            {courts.length > 0 && (
              <div>
                <p className="text-[10px] font-bold text-[#1a1a1a]/40 tracking-wider mb-2">DATE</p>
                <div className="flex gap-2 overflow-x-auto no-scrollbar">
                  {dates.map((d) => (
                    <button
                      key={d.value}
                      onClick={() => setSelectedDate(d.value)}
                      className={`flex-shrink-0 flex flex-col items-center px-3.5 py-2 rounded-lg border transition-all min-w-[3.5rem] ${
                    ...
                    >
                      ...
                      <span className="font-stat text-base leading-none mt-0.5">{d.day}</span>
        ...
        {!loading && selectedVenue && (
          courts.length === 0 ? (
            <div className="text-center py-16 rounded-2xl" style={frostedCard}>
            ...
            <>
              ...
              <div className="flex items-center justify-between gap-4 pb-1">
                <div className="flex items-center gap-2.5 min-w-0">
                  <h2 className="font-heading text-lg font-bold text-[#1a1a1a] truncate">{selectedCourt?.name}</h2>
                  {selectedCourt && <SportBadge sport={selectedCourt.sport} />}
                  <span className="font-stat text-sm text-brand-orange flex-shrink-0">฿{selectedCourt?.price_per_hour}/hr</span>
                </div>
                <button
                  onClick={() => setShowAddForm(!showAddForm)}
                  className="flex-shrink-0 bg-brand-orange text-white px-4 py-2 rounded-lg text-xs font-semibold flex items-center gap-1.5"
                >
                  <Plus size={14} />
                  Add Slot
                </button>
              ...
              {/* Add slot form */}
              {showAddForm && (
                <div className="rounded-2xl p-4 animate-fade-in space-y-3" style={frostedCard}>
                <div className="flex items-center justify-between">
                  <h3 className="font-heading text-sm font-bold text-[#1a1a1a]">NEW SLOT</h3>
                  <span className="text-xs text-[#1a1a1a]/40 font-light">{format(new Date(selectedDate), "EEE, d MMM")}</span>
                </div>
                  <div className="grid grid-cols-3 gap-2">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full border border-[#1a1a1a]/12 rounded-lg px-2 py-2 text-sm text-[#1a1a1a] focus:outline-none focus:border-brand-orange bg-white"
                      >
                  ...
                  <div className="flex gap-2 pt-1">
                    <button onClick={() => setShowAddForm(false)} className="flex-1 py-2 border border-[#1a1a1a]/12 rounded-lg text-sm text-[#1a1a1a] font-semibold">Cancel</button>
                    <button onClick={createSlot} className="flex-1 py-2 bg-brand-orange text-white rounded-lg text-sm font-semibold">Create Slot</button>
                  </div>
                </div>
              )}
              ...
              {slots.length === 0 ? (
                <div className="text-center py-12 rounded-2xl" style={frostedCard}>
                  ...
                  <p className="font-heading text-base text-[#1a1a1a]">No slots for this date</p>
                  <p className="text-[#1a1a1a]/40 text-xs font-light">Click Add Slot to create availability.</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {slots.map((slot) => {
                    ...
                    return (
                      <div key={slot.id} className="rounded-xl px-4 py-3 flex items-center gap-3" style={frostedCard}>
                        {/* Time */}
                        <p className="font-heading text-sm font-bold text-[#1a1a1a] flex-shrink-0 min-w-[7rem]">{slot.start_time} – {slot.end_time}</p>
                        {/* Price */}
                        <p className="font-stat text-sm text-brand-orange flex-shrink-0">฿{slot.price}</p>
                        {/* Status */}
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${cfg.cls} flex-shrink-0`}>{cfg.label}</span>
                        {/* Actions */}
                          ...
                          {!isBooked ? (
                            <>
                              <button
                                ...
                                className="p-1.5 rounded-lg text-[#1a1a1a]/40 hover:text-brand-orange hover:bg-brand-orange/10 transition-all disabled:opacity-40"
                                title={slot.status === "blocked" ? "Unblock" : "Block"}
                              >
                                {slot.status === "blocked" ? <CheckCircle2 size={15} /> : <Ban size={15} />}
                              </button>
                              <button
                                onClick={() => deleteSlot(slot)}
                                disabled={acting === slot.id}
                                className="p-1.5 rounded-lg text-[#1a1a1a]/40 hover:text-destructive hover:bg-destructive/10 transition-all disabled:opacity-40"
                                title="Delete"
                              >
                                <Trash2 size={15} />
                              </button>
                            ...
                            <span className="text-[10px] text-[#1a1a1a]/30 font-light">In use</span>
                          )}
                        </div>
                      </div>
                    );
